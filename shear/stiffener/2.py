...
if lamb>lamb_p:
    print('lambda > lambda P, não compacta, colocando enrijecedores...')
    secao = 'compacta'
    if secao=='compacta':
        kv = (lamb**2)*fy/(1.21*E)
        # a = np.sqrt(E*(tw**2)*6.05/(fy*(hw**2)-E*(tw**2)*6.05))
    ...
    if lamb<=lamb_p:
        print('Seção compacta ',lamb,' <= ',lamb_p)
        V_Rd_enrij = Vpl/gamma_a1
    elif lamb<=lamb_r:
        print('Seção semicompacta ',lamb,' <= ',lamb_r)
        V_Rd_enrij = lamb_p*Vpl/(lamb*gamma_a1)
    else:
        print('Seção esbelta ',lamb,' > ',lamb_r)
        V_Rd_enrij = 1.24*((lamb_p/lamb)**2)*Vpl/gamma_a1
    print('V_Rd com enrijecedor = ',V_Rd_enrij)