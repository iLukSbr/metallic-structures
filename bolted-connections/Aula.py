...
lf_borda = 4.6 - dfe/2 #cm VER DESENHO
#Distância reta da borda de um furo à borda de outro furo
lf_interno = 12.64 - dfe #cm VER DESENHO
#Fissuração total
lf = 2*(2*lf_interno + lf_borda)
...
#Chapa tipo 2
        ...
        Fcrd_lf = (Cpl*fu/gamma_a2)*t2*lf
        Fcrd_db = (Cfp*fu/gamma_a2)*t2*db*n_parafusos
        Fcrd = n_chapas2*min(Fcrd_lf , Fcrd_db)
    if Fcrd >= Fsd:
