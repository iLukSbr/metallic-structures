...
#================================================================
#Carga cortante
Fg = 400 #kN permanente
Fq = 0 #kN variável
gamma_g = 1.5 #CARGA DE EQUIPAMENTOS
gamma_q = 0 #CARGA DE 
...
#Resistência última
fub = 82.5 #kN/cm² parafuso tipo A325
gamma_a2 = 1.35
...
lf_borda1 = 3.81 - dfe/2 #cm VER DESENHO
#Distância reta da borda de um furo à borda de outro furo
lf_interno1 = 7.62 - dfe #cm VER DESENHO
lf_interno2 = 2*3.81 - dfe
#Fissuração total
lf1 = 2*(lf_interno1 + lf_borda1)
lf2 = 4*lf_interno1 + 2*lf_interno2 + 4*lf_borda1
...
Ant1 = 2*(3.81 - dfe/2)*t1*n_chapas1 #cm² VER DESENHO
Ant2 = 2*(3.81 - dfe/2)*t2*n_chapas2 #cm² VER DESENHO

#Área bruta sujeita ao cisalhamento (medida paralela à carga)
Agv1 = 2*(3.81 + 6.985)*t1*n_chapas1 #cm² VER DESENHO
Agv2 = 2*(3.81*4 + 6.985*2)*t2*n_chapas2 #cm² VER DESENHO

#Área líquida sujeita ao cisalhamento (medida paralela à carga)
Anv1 = Agv1 - 2*1.5*dfe*t1*n_chapas1 #cm² VER DESENHO
Anv2 = Agv2 - 2*4*dfe*t2*n_chapas2 #cm² VER DESENHO
...
#Espaçamento entre bordas internas dos furos
    ...
    if min(lf_interno1,lf_interno2) >= db:
        print('lf_interno=',min(lf_interno1,lf_interno2),'>= db=',db,', espaçamento entre furos atende ao mínimo.')
    else:
        print('lf_interno=',min(lf_interno1,lf_interno2),'< db=',db,', espaçamento entre furos NÃO atende ao mínimo.')
    print(' ')
...
#     Fsd = (Fsd_calc/n_parafusos2)*(1 + alfa*delta*b/((1+alfa*delta)*a))

#================================================================
#Força resistente ao cisalhamento nos parafusos
if cortante == 's':
    print('Força resistente de cisalhamento dos parafusos:')
    n_parafusos2 = 2*np.ceil((Fsd*gamma_a2/(n_planoscorte*Cpc*Ab*fub))/2)
    print('n_parafusos2=',n_parafusos2)
    n_parafusos1 = n_parafusos2/2
    print('n_parafusos1=',n_parafusos1)
    print(' ')
    # if lpega > 5*db:
...
if tracao == 's':
    ...
    n_parafusos1 = np.ceil(Fsd*gamma_a2/(0.75*n_planoscorte*Ab*fub))
    print('n_parafusos1=',n_parafusos1)
    # if Ftrd >= Fsd:
...
#Chapa tipo 1
    if min(n_chapas1*t1,n_chapas2*t2) == n_chapas1*t1 or n_chapas2 == 0:
        print('Limitada pela chapa tipo 1:')
        Fcrda = (Cpl*fu/gamma_a2)*t1*lf1
        Fcrdb = (Cfp*fu/gamma_a2)*t1*db*n_parafusos1
        Fcrd = n_chapas1*min(Fcrda , Fcrdb)
#Chapa tipo 2
    elif n_chapas2 > 0 and min(n_chapas1*t1,n_chapas2*t2) == n_chapas2*t2:
        print('Limitada pela chapa tipo 2:')
        Fcrdc = (Cpl*fu/gamma_a2)*t2*lf2
        Fcrdd = (Cfp*fu/gamma_a2)*t2*db*n_parafusos2
        Fcrd = n_chapas2*min(Fcrdc , Fcrdd)
    if Fcrd >= Fsd:
